...
api.interceptors.response.use((response) => response, (error) => { if (error.response?.status === 401 && typeof window !== 'undefined') { getAuthSnapshot().logout(); window.location.assign('/login?expired=1') } return Promise.reject(error) })
export default api
